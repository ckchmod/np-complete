...
SHIPPED 2026-05-29 — **LIVE at https://ckchmod.github.io/np-complete/** (Pages, `main` root). The cloud `/code-review ultra` couldn't run (no commits ahead of `main`), so the local `/code-review max` ran as the gate: a 9-angle Workflow over `cb7b859..HEAD` → 18 findings → 6 root causes fixed in `96e52ad` (reduced-motion animationend leaks, strikeFlash bubbling + is-strike reset, overlay stacking via anyOverlayOpen(), aria-live HUD spam, generator determinism comment); 5 refuted. Plus HUD relabel par→optimal / drop live pb (`534c30a`), `.nojekyll` (`5f6daa1`), repo public, Pages enabled. 47/47 tests. Deferred (pre-existing, not regressions): overlay focus-trap/Escape a11y; showOverlay/hideOverlay helper; pause/resume unit test (DOM-bound, zero-dep project). Now in maintenance/iteration.
